...
if [ -z "$lazygit_VERSION" ] || [ -z "$BUILD_VERSION" ]; then
    echo "Usage: $0 <lazygit_version> <build_version>"
    echo "Example: $0 0.44.1 1"
    exit 1
fi

PACKAGE_NAME="lazygit"
ORIG_TARBALL="${PACKAGE_NAME}_${lazygit_VERSION}.orig.tar.gz"
BUILD_DIR="${PACKAGE_NAME}-${lazygit_VERSION}"

echo "Creating Debian/Ubuntu source packages for lazygit ${lazygit_VERSION}-${BUILD_VERSION}..."
...
if [ ! -f "$ORIG_TARBALL" ]; then
    ...
    wget -q "https://github.com/jesseduffield/lazygit/archive/refs/tags/v${lazygit_VERSION}.tar.gz" -O "$ORIG_TARBALL"
...
build_source_package() {
    local dist=$1
    local FULL_VERSION="${lazygit_VERSION}-${BUILD_VERSION}~${dist}"
    ...
    rm -rf "$BUILD_DIR"
    tar -xf "$ORIG_TARBALL"
    # GitHub archives extract as lazygit-0.x.y/ which matches our BUILD_DIR
...
lazygit (${FULL_VERSION}) ${dist}; urgency=medium

  * New upstream release ${lazygit_VERSION}.
